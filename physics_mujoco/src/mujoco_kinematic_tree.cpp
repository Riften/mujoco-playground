...
namespace physics_mujoco {
    ...
    bool KinematicTree::addLinkRecursive(const mjModel * model, int link_id) {
        ...
        for(auto child : body_info_[link_id].children_id) {
            addLinkRecursive(model, child);
        }

        return true;
    }

    KinematicTree::KinematicTree(const mjModel *model, const std::string &first_link, const std::string &last_link) {
        // Fetch body id
         ...
         */
        addLinkRecursive(model, first_link_id);
    }
}